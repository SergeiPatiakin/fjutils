...
def bitbucket_create(username, reponame, password):
    ...
    user_and_pass = b64encode(bytes("{0}:{1}".format(username, password), encoding='ISO-8859-1')).decode("ascii")
    headers = {
    ...
    conn.request('POST', '/2.0/repositories/{0}/{1}'.format(username, reponame), headers=headers, body=body)
